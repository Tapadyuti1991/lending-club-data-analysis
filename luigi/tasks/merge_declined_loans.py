...
class MergeDataDownloaded(luigi.Task):
    ...
    def run(self):
        ...
        cwd = os.getcwd()
        folder = os.path.join(cwd,"Data/DECLINED_LOAN_DATA")
        output_folder = os.path.join(cwd,"Data/")
        print("Started : Prepare Declined data files for merge")
        df_list = []
        
        #check inorder to avoid inclusion in merge to be tested to-do $$$$
        mergedFile = output_folder+"/CombinedDeclinedData.csv" #check / \\  >> \
        ...
        combined_csv.to_csv(output_folder+"/CombinedDeclinedData.csv", index=False ) #check / \\ \
        
        print("Finished : Merging declined data files to CombinedDeclinedData.csv ")
